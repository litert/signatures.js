...
class EcdsaSigner implements dG.ISigner {

    public readonly hashAlgorithm!: dL.IAlgorithms;

    public readonly signAlgorithm = 'ecdsa';
    ...
    public constructor(
            ...
            if (typeof publicKey === 'string' || publicKey instanceof Buffer) {
                ...
                try {
                    ...
                    publicKey = NodeCrypto.createPublicKey({
                        key: publicKey,
                        format: publicKey instanceof Buffer ? 'der' : 'pem',
                        type: 'spki',
                    });
                }
                catch (e) {
        ...
        if (privateKey && publicKey) {
            ...
            const kCN = (privateKey as NodeCrypto.KeyObject).asymmetricKeyDetails!.namedCurve;
            const pCN = (publicKey as NodeCrypto.KeyObject).asymmetricKeyDetails!.namedCurve;
            if (kCN !== pCN) {
 ...
 *
 * @returns A new ECDSA signer instance.
 *
 * @throws `E_NO_KEY_PROVIDED` If neither publicKey nor privateKey is provided.
 * @throws `E_INVALID_PUBLIC_KEY` If the provided public key is invalid or not of type 'ec'.
 * @throws `E_INVALID_PRIVATE_KEY` If the provided private key is invalid or not of type 'ec'.
...
export function createSigner(
    ...
    return new EcdsaSigner(hashAlgo, publicKey, privateKey, opts);
}
